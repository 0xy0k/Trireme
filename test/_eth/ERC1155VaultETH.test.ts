...
describe('ERC1155 Vault ETH', () => {
  ...
  before(async () => {
    ...
    provider = <MockERC1155ValueProviderETH>await upgrades.deployProxy(
      providerFactory,
      [
  ...
  describe('Initializing', () => {
    beforeEach(async () => {
      const factory = await ethers.getContractFactory('ERC1155VaultETH');
      vault = <ERC1155VaultETH>await upgrades.deployProxy(factory, [
        stablecoin.address,
        mockNFT.address,
        tokenIndex,
        provider.address,
  ...
  describe('Borrow TriETH', () => {
    ...
    it('should be able to borrow triremeETH against his collateral on the vault', async () => {
    ...
    it('should cut orgnization fee in triremeETH when borrowing', async () => {
