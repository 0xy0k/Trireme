...
abstract contract ERC20ValueProviderBTC is
    ...
    AccessControlUpgradeable
{
    ...
    function getCreditLimitBTC(
        ...
        return _creditLimitRate.calculate(getPriceBTC(_colAmount));
    }
    ...
    function getLiquidationLimitBTC(
        ...
        return _liquidationLimitRate.calculate(getPriceBTC(_colAmount));
    }

    /// @return The value for the collection, in BTC.
    function getPriceBTC(uint256 colAmount) public view returns (uint256) {
        uint256 price = getPriceBTC();
        return (price * colAmount) / (10 ** token.decimals());
    }

    /// @return The value for the collection, in BTC.
    function getPriceBTC() public view virtual returns (uint256) {
